...
Visualizer::Visualizer(const std::string &node_name, int node_id)
    : Node(node_name), node_id_(node_id) {
  ...
  ego_vehicle_pub_ = this->create_publisher<visualization_msgs::msg::MarkerArray>(ego_vehicle_vis_topic, 10);
  obstacle_map_pub_ = this->create_publisher<visualization_msgs::msg::MarkerArray>(obstacle_map_vis_topic, 10);
  surrounding_lane_net_pub_ = this->create_publisher<visualization_msgs::msg::MarkerArray>(surrounding_lane_net_vis_topic, 10);
  local_lanes_pub_ = this->create_publisher<visualization_msgs::msg::MarkerArray>(local_lanes_vis_topic, 10);
  behavior_vis_pub_ = this->create_publisher<visualization_msgs::msg::MarkerArray>(ego_vehicle_behavior_topic, 10);
  pred_traj_openloop_vis_pub_ = this->create_publisher<visualization_msgs::msg::MarkerArray>(pred_traj_openloop_topic, 10);
  pred_intention_vis_pub_ = this->create_publisher<visualization_msgs::msg::MarkerArray>(pred_intention_topic, 10);
  surrounding_vehicle_vis_pub_ = this->create_publisher<visualization_msgs::msg::MarkerArray>(surrounding_vehicle_topic, 10);
  speed_limit_vis_pub_ = this->create_publisher<visualization_msgs::msg::MarkerArray>(speed_limit_topic, 10);
  ego_to_map_tf_ = std::make_unique<tf2_ros::TransformBroadcaster>(this);
}
...
void Visualizer::VisualizeDataWithStamp(const rclcpp::Time &stamp, const SemanticMapManager &smm) {
  VisualizeEgoVehicle(stamp, smm.ego_vehicle());
  VisualizeObstacleMap(stamp, smm.obstacle_map());
  VisualizeSurroundingLaneNet(stamp, smm.surrounding_lane_net(), std::vector<int>());
...
void Visualizer::VisualizeDataWithStampForPlayback(const rclcpp::Time &stamp, const SemanticMapManager &smm, const std::vector<int> &deleted_lane_ids) {
  VisualizeEgoVehicle(stamp, smm.ego_vehicle());
  VisualizeObstacleMap(stamp, smm.obstacle_map());
...
void Visualizer::VisualizeObstacleMap(
  ...
  common::VisualizationUtil::GetRosOccupancyGridUsingGripMap2D(
      obstacle_map, rclcpp::Time::now(), &occ_map);
