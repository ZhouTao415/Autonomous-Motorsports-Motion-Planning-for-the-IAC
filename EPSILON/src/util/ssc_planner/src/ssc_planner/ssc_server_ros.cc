...
SscPlannerServer::SscPlannerServer(const rclcpp::NodeOptions &options, int ego_id)
    : Node("ssc_planner_server", options), ego_id_(ego_id) {
  p_input_smm_buff_ = new moodycamel::ReaderWriterQueue<SemanticMapManager>(
      config_.kInputBufferSize);
  p_smm_vis_ = std::make_unique<semantic_map_manager::Visualizer>(options, ego_id);
  p_ssc_vis_ = std::make_unique<SscVisualizer>(this->shared_from_this(), ego_id);
}

SscPlannerServer::SscPlannerServer(const rclcpp::NodeOptions &options, double work_rate,
                                   int ego_id)
...
void SscPlannerServer::PublishData() {
  ...
  // ssc visualization
  {
    ...
    RCLCPP_INFO(this->get_logger(), "ssc vis all time cost: %lf ms", timer.toc());
  }
  ...
  // trajectory feedback
  {
    ...
    if (use_sim_state_) {
      RCLCPP_INFO(this->get_logger(), "use_sim_state_: true.\n");
    } else {
      RCLCPP_INFO(this->get_logger(), "use_sim_state_: false.\n");
    }
    ...
    if (use_sim_state_) {
      ...
      decimal_t ct = executing_traj_->begin() + num_cycles * plan_horizon;
      {
        ...
        if (executing_traj_->GetState(ct, &state) == kSuccess) {
          ...
          RCLCPP_ERROR(
              this->get_logger(),
              "Cannot evaluate state at %lf with begin %lf.",
...
ErrorType SscPlannerServer::FilterSingularityState(
  ...
  if (fabs(filter_state->velocity) < singular_velocity &&
    ...
    RCLCPP_WARN(this->get_logger(), "Filter angle to %lf.", hist.back().angle);
  }
  return kSuccess;
}
...
void SscPlannerServer::Start() {
  ...
  RCLCPP_INFO(this->get_logger(), "Planner server started.");
  is_replan_on_ = true;
...
void SscPlannerServer::PlanCycleCallback() {
  ...
  RCLCPP_INFO(this->get_logger(), ">>>>>>>current time %lf.", current_time);
  if (executing_traj_ == nullptr || !executing_traj_->IsValid() ||
    ...
    RCLCPP_INFO(this->get_logger(),
        "init plan success with stamp: %lf and angle %lf.",
        global_init_stamp_, last_smm_.ego_vehicle().state().angle);
    return;
  }

  if (current_time > executing_traj_->end()) {
    RCLCPP_INFO(this->get_logger(), "Current time %lf out of [%lf, %lf].",
...
void SscPlannerServer::Replan() {
  ...
  RCLCPP_INFO(this->get_logger(),
      "init stamp: %lf, plan horizon: %lf, num cycles %d.",
      global_init_stamp_, plan_horizon, num_cycles_ahead);
  RCLCPP_INFO(this->get_logger(),
      "Replan at cur time %lf with executing traj begin time: %lf to rounded t: %lf.",
      cur_time, executing_traj_->begin(), t);

  if (executing_traj_->GetState(t, &desired_state) != kSuccess) {
    RCLCPP_WARN(this->get_logger(), "Cannot get desired state at %lf.", t);
    return;
  }
  RCLCPP_INFO(this->get_logger(),
      "t %lf, desired state (x,y,v,a,theta):(%lf, %lf, %lf, %lf, %lf).",
  ...
  if (planner_.RunOnce() != kSuccess) {
    RCLCPP_WARN(this->get_logger(), "Ssc planner core failed in %lf ms.",
  ...
  RCLCPP_INFO(this->get_logger(), "Ssc planner succeed in %lf ms.",
         time_profile_tool_.toc());
